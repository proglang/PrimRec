...
$n+2$ then $f := Pr (g,h)$ is a primitive recursive function of arity $n+1$ that is defined as follows.
\begin{align*}
...
catamorphism-operator for inductive types,
...
constructor of inductive types  {\Acon}.  The catamorphism eliminator
is {\ACt}, and the primitive recursive eliminator is {\AP}.
In the definition of {\Acon}, {\ACt}, and {\AP}, we find that $G :$~\ATy1, a type with a
...
catamorphism (aka the catamorphism elimination operator) at this type:
\ccDataPRF
The catamorphism operator invokes $h$ on the result of type $T$ of its
...
In PR-FO, we can easily show that {\AP} and {\ACt} are interdefinable.
...
first component of the argument must evaluate to some \Acon~$x$, so the $x$ corresponds
...
\ccFunMkvec
\ccDefNatToInd
The encodings of the constructors {\ACZ} and {\ACS} compose {\Acon} with the left and right
...
must have the form $\Acon\ x$, for some $x : \Aind~H \Leftarrow S$.
...
morally $\Acon~ (\Afmap~H'~f~x)$ should do the job. Unfortunately,
