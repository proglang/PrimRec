...
data _≈ᴵ_ : ∀ {T U : TY FO} → T →ᴾ U → T →ᴾ U → Set where
  ...
  `case-congᴵ : ∀ {A B D : TY FO}
    ...
    f ≈ᴵ f′ → g ≈ᴵ g′ →
    `case f g ≈ᴵ `case f′ g′
  Pr-congᴵ : ∀ {A B : TY FO} {G : Ty FO 1}
    {h h′ : (G [ A `× ind G ]) `× B →ᴾ A} →
    h ≈ᴵ h′ →
    Pr {G = G} {T = A} {U = B} h ≈ᴵ Pr {G = G} {T = A} {U = B} h′
  ...
  Pr-β-branch : ∀ {k n} (rs : Vec ℕ k)
    (steps : (i : Fin k) →
      vec (Tree rs) ((lookup rs i + lookup rs i) + n) →ᴾ Tree rs)
    {X : TY FO} (i : Fin k)
    (children : Vec (X →ᴾ Tree rs) (lookup rs i))
    (parameters : Vec (X →ᴾ Tree rs) n) →
    let rec = Pr {G = Branches rs} {T = Tree rs} {U = vec (Tree rs) n}
  ...
  Pr-β-zero : ∀ {n X} (g : vec Nat n →ᴾ Nat)
    (h : vec Nat (2 + n) →ᴾ Nat)
    (parameters : Vec (X →ᴾ Nat) n) →
    let rec = Pr {G = NatF} {T = Nat} {U = vec Nat n} (nat-Pr-handler g h)
        parameter-tuple = tupleᴾ parameters
        zeroᴾ = C (C (con {G = NatF}) ι₁) (`⊤ {T = X})
    in
    C rec (`# zeroᴾ parameter-tuple) ≈ᴵ C g parameter-tuple

  Pr-β-suc : ∀ {n X} (g : vec Nat n →ᴾ Nat)
    (h : vec Nat (2 + n) →ᴾ Nat)
    (counter : X →ᴾ Nat)
    (parameters : Vec (X →ᴾ Nat) n) →
    let rec = Pr {G = NatF} {T = Nat} {U = vec Nat n} (nat-Pr-handler g h)
        parameter-tuple = tupleᴾ parameters
        successor = C (C (C (con {G = NatF}) ι₂) π₁)
                      (`# counter (`⊤ {T = X}))
  ...
  Ct-β-zero : ∀ {n X} (g : vec Nat n →ᴾ Nat)
    (h : vec Nat (1 + n) →ᴾ Nat)
    (parameters : Vec (X →ᴾ Nat) n) →
    let rec = Pr {G = NatF} {T = Nat} {U = vec Nat n} (nat-Ct-handler g h)
        parameter-tuple = tupleᴾ parameters
        zeroᴾ = C (C (con {G = NatF}) ι₁) (`⊤ {T = X})
    in
    C rec (`# zeroᴾ parameter-tuple) ≈ᴵ C g parameter-tuple

  Ct-β-suc : ∀ {n X} (g : vec Nat n →ᴾ Nat)
    (h : vec Nat (1 + n) →ᴾ Nat)
    (counter : X →ᴾ Nat)
    (parameters : Vec (X →ᴾ Nat) n) →
    let rec = Pr {G = NatF} {T = Nat} {U = vec Nat n} (nat-Ct-handler g h)
