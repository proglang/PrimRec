...
module Core.Contextual.Equations.PRFO where

open import Core.Contextual.PRFO public
...
data _≈_ : ∀ {Γ A} → Γ ⊢ A → Γ ⊢ A → Set where
  ...
  dist-undist : ∀ {A B C}
    → cut (dist-+-× {A = A} {B = B} {C = C}) undist-+-× ≈ var
  undist-dist : ∀ {A B C}
