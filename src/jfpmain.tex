...
The following functions and lemmas are useful in all parts of this proof. They relate the evaluation of term and the term behind some binders.

\prepLambdas

\input{latex/VecProperties}

Here \AgdaFunction{++r} is takes two vectors, reverses the first and appends it to the second.
\appendR
...
The idea behind these defintions is that every element of \AgdaDatatype{Exp n m} can be transformed to a closed term by wrapping the term behind \AgdaBound{n} binders. By the theorem \AgdaFunction{prepLambdasEval} we that evaluating this  expression on reverse-concatenation of the context and the arguments gives the same result as evaluating the old expression.

%now about the semantic of this new term. 

When we define the function \AgdaFunction{sTtoPR} we have to consider that the returning primtive recursive functions will be evaluated not only on the arguments but on the reverse-concatenation of the context and the arguments.

For example the sucessor function has to explicitly ignore the context.
\sTtoPRSuc
That is done with a projection that returns the last argument of the input.
\sTtoPRSoundSuc

Then the proof can be reduced to a simple therem about vectors.
\lookupFromN
