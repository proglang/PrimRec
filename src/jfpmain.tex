...
First we define functions that return closed terms, that hat have a similiar semantic as the constructors of the primitive recursion.
It is straight forward to define them in Agda, but to get a understanding of the idea of this proof it is enough to know, what we want to express with them.
\begin{enumerate}
	\item $$\lambda x_1 \dots x_{n} \rightarrow 0 $$
	\item $$\lambda x_1 \dots x_i \dots x_{n} \rightarrow x_i $$
...
That we prooved, that they work as expected. The function that generates the term that mimics the composition is called \AgdaFunction{generalComp}
