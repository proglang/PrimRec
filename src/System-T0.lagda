...
eval (Var x) ctx args = lookup ctx x
eval (Lam e) ctx (x ∷ args) = eval e (x ∷ ctx) args
eval CZero ctx args = 0
eval Suc ctx [ x ] = suc x
...
mapEvalVarsEq  {o} {n} {m}  zs xs ys  = map (λ arg → eval arg (zs ++ xs ++r ys) []) (map Var (mkFinvec' n m o)) 
...
evalGeneralCompHelper {n} {m} args gs = 
                        (map (λ arg → eval arg (fastReverse args) [])(map (applyToVars {m} {zero}) (map (raiseExP m) gs))) 
...
\begin{code}[hide]
evalGeneralComp {n} {m} f gs args = (evalClosed (generalComp f gs) args)
...
evalParaTHelper3 {n} x counter acc h args =     eval (applyToVars' {2} {n} {1} (App (App (raiseExP (suc (suc (suc n))) h) (Var (suc zero)))(Var zero)))  (counter ∷ acc ∷ fastReverse (x ∷ args)) [] 
...
evalParaT {n} {x} g h args = (evalClosed (paraT g h) (x ∷ args)) 
                        ≡⟨⟩ 
