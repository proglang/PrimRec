...
open System-T0.Exp
open import EvalPrConstructor using (para; paraNat'; evalPr≡paraNat')
...
prToST′  (Pr g h) = convPR g h
prToST′  (Ct {n = n} g h) = convPR g (C h (dropCounter n))
...
convParaSound : ∀ {n : ℕ} (g : PR n) (h : PR (suc (suc n))) (args : Vec ℕ (suc n) ) → evalClosed (prToST′ (Pr g h)) args ≡ eval (Pr g h) args
...
embeddPR-ST-Sound  (Pr g h) vs = convParaSound g h vs
embeddPR-ST-Sound  (Ct g h) vs = trans (convParaSound g (C h (dropCounter _)) vs) (sym (Ct⇒Pr-sound g h vs))
...
convParaSound g h (x ∷ args) = begin
                        (evalClosed (prToST′ (Pr g h)) (x ∷ args))
                                ≡⟨⟩
